...
In fact, following the terminology in \citep{gsellYieldTransientEffects1981}, the strain rate equation for a given kinetic process can be written as
\begin{equation}
  \dot\gamma = \bar{b}\rho_m \bar{v},
\end{equation}
where $\bar b$ is the mean length of the plastic wave, $rho_m$ the length density of plastic waves and $\bar v$ their mean velocity.
Since the plastic waves are thermally released, their mean veloctiy $\bar v$ is given by their mean free path before they are traped again, $y_m$, the frequency of attempst to cross the boundary and the probability ...
...
Zaïri et al. \cite{zairiElastoviscoplasticConstitutiveEquations2007} propose similar rate equations for partial contributions, $\hat \tau^{(1)}$ and $\hat \tau^{(2)}$m, to the athermal strength, $\hat \tau$.
...
The rate equation for the athermal strength, $s$, is chosen according to Equation~\eqref{eq:rate_equation_bpa}.
\begin{equation}
